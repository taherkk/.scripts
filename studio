...
if [ -z "$JDK" -a -s "$HOME/.AndroidStudio3.1/config/studio.jdk" ]; then
  USER_JRE=`"$CAT" $HOME/.AndroidStudio3.1/config/studio.jdk`
  if [ ! -d "$USER_JRE" ]; then
...
for VM_OPTIONS_FILE in \
    ...
    $HOME/.AndroidStudio3.1/config/studio$BITS.vmoptions \
    $IDE_HOME.vmoptions \
...
if [ "$IS_EAP" = "true" ]; then
  ...
  if [ -r "$AGENT_PATH" ]; then
    AGENT="-agentpath:$AGENT_PATH=probe_disable=*,disablealloc,disabletracing,onlylocal,disableexceptiontelemetry,delay=10000,sessionname=AndroidStudio3.1"
  fi
fi
...
"$JAVA_BIN" \
  ...
  -Didea.paths.selector=AndroidStudio3.1 \
